...
export const useGrades = (marks: Grading | undefined | null) => {
  ...
  useEffect(() => {
    ...
    const currentGradeWeightedSum = completedAssignments.reduce(
      (acc, { mark, weight }) => acc + parseFloat(mark) * parseFloat(weight),
      0
    );
    ...
    const currentGradeAvg =
      currentWeightSum > 0 ? currentGradeWeightedSum / currentWeightSum : 0;
